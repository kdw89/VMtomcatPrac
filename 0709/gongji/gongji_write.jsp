...
<%
	...
	String saveDir = application.getRealPath("/0709/gongji/upload/");
	...
	if(id.equals("INSERT") == false) {
		sql = "select filepath1, filepath2, filepath3 from resort_gongji where id = ?";
...
<%
	...
	if(id.equals("INSERT")){
		sql = "insert into resort_gongji (title, content, date, filepath1, filepath2, filepath3) values(?, ?, now(), ?, ?, ?)";
		pstmt = conn.prepareStatement(sql);
		...
		sql = "update resort_gongji set title=?, content=?, filepath1=?, filepath2=?, filepath3=? where id=?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1,title);
		pstmt.setString(2,content);
		pstmt.setString(3,filename1);

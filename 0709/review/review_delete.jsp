...
<%
	...
	String sql = "";
	sql = "select relevel, recnt from resort_review where rootid=? and recnt > ? order by recnt asc";
	...
	sql = "select filepath1, filepath2, filepath3 from resort_review where rootid=? AND recnt BETWEEN ? AND ?";
	pstmt = conn.prepareStatement(sql);
	pstmt.setString(1,root);
	...
	while(rset2.next()){
		String path1 = "/var/lib/tomcat8/webapps/ROOT/0709/review/upload/" + rset2.getString(1);
		String path2 = "/var/lib/tomcat8/webapps/ROOT/0709/review/upload/" + rset2.getString(2);
		String path3 = "/var/lib/tomcat8/webapps/ROOT/0709/review/upload/" + rset2.getString(3);
	...
	sql = "delete from resort_review where rootid=? and recnt between ? and ?";
	pstmt = conn.prepareStatement(sql);
	pstmt.setString(1,root);
	pstmt.setString(2,recnt);
	pstmt.setInt(3,recnt2+temp);
	pstmt.executeUpdate();
	out.println(root);
	sql = "update resort_review set recnt=recnt-? where rootid=? and recnt >=?";
	pstmt = conn.prepareStatement(sql);
	pstmt.setInt(1,temp+1);
	pstmt.setString(2,root);
	pstmt.setString(3,recnt);
	pstmt.executeUpdate();
...
%>
<script>location.href = "review_list.jsp"; </script>
</body>
</html>
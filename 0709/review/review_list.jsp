...
<%
	...
	if(pageNum != null)	{
		try{
			...
			out.println("<script>alert('잘못된 접근입니다.'); location.href='review_list.jsp';</script>");
		}
	}
...
<%
	while (rset.next()) {
			...
			out.println("<a href='review_view.jsp?key="+rset.getInt(1)+"'>"+rset.getString(2)+"</a></div></span>"+newPost+"</td>");
...
if (pageListStart != 1) {			//1페이지가 포함되었을땐 <<, <가 나타나지 않는다
	out.println("<a href=review_list.jsp?pageNum="+1+" class='btn' style='font-size:2em'>&lt;&lt;</a>");	//맨 앞으로
	out.println("<a href=review_list.jsp?pageNum="+(pageListStart-1)+" class='btn' style='font-size:2em'>&lt;</a>");		//10페이지 전으로
}
for(int i=pageListStart; i<pageListStart+10; i++){		//10개 페이지 링크 출력, 페이지번호와 시작 번호를 매칭...수열
	...
	if(i == currentPage)	out.println("<a href=review_list.jsp?pageNum="+(i)+" class='btn btn-success' style='font-size:2em'><b>"+(i)+"</b></a>");
	else	out.println("<a href=review_list.jsp?pageNum="+(i)+" class='btn btn-info' style='font-size:2em'>"+(i)+"</a>");

}
if (20*pageListStart < total-180) {		//마지막 페이지에서는 >, >>가 안나타나도록
	out.println("<a href=review_list.jsp?pageNum="+(pageListStart+10)+" class='btn' style='font-size:2em'>&gt;</a>");		//10페이지 뒤로
	if(total%20 == 0) out.println("<a href=review_list.jsp?pageNum="+(total/20)+" class='btn' style='font-size:2em'>&gt;&gt;</a><br>");
	else out.println("<a href=review_list.jsp?pageNum="+(total/20+1)+" class='btn' style='font-size:2em'>&gt;&gt;</a><br>");	//마지막 페이지로
}

%><br>
<table width=850 style="background-color: transparent">
<tr>
<td width=750></td>
<td><input type=button value="신규" class="btn btn-warning btn-lg" OnClick="window.location='review_insert.jsp'"></td>
</tr>
</table>
</div>
</center>

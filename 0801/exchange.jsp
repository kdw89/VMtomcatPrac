...
cal.add(cal.DATE,-30);
%>
<select name="cur" id="cur" onchange="getCur(this)" style="font-size:large; text-align:center; width:200px; height:40px;">
...
//out.println(str_days + "<br>" + str_ttb + "<br>" + str_tts + "<br>");
//in.close();
...
function getCur(selectObject) {
	...
	switch(value) {
		case "USD" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = USD_tts[j];
			}		
			break;
		case "CNH" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = CNH_tts[j];
			}			
			break;
		case "JPY" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = JPY_tts[j];
			}
			break;
		...
		case "EUR" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = EUR_tts[j];
			}
			break;
		...
		case "GBP" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = GBP_tts[j];
			}
			break;
		...
		case "AUD" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = AUD_tts[j];
			}
			break;
		...
		case "CAD" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = CAD_tts[j];
			}
			break;
		...
		case "NZD" :
			...
			for(var j=0; j<30; j++) {
				myLine.data.datasets[1].data[j] = NZD_tts[j];
			}
			break;
	}
